...
class dual_pole_rx(gr.top_block, Qt.QWidget):
    def __init__(self):
        ...
        # Note: sync_0 has osps=2, setting up perfect 2 SPS for the equalizer
        ...
        # --- QPSK Linear Equalizer ---
        self.adpt_alg = digital.adaptive_algorithm_cma(
            digital.constellation_qpsk().base(), 0.01, 1
        ).base()
        
        self.digital_linear_equalizer_0 = digital.linear_equalizer(
            15, 2, self.adpt_alg, True, [], 'corr_est'
        )
        ...
        # Connections
        # Channel 0 (QPSK) flows: Source -> AGC -> FLL -> Clock Sync (2 SPS) -> Linear EQ (1 SPS) -> Costas Loop
        self.connect((self.uhd_usrp_source_0, 0), self.agc_0, self.fll_0, self.sync_0, self.digital_linear_equalizer_0, self.costas_0)
        
        # Channel 1 (BPSK) flows normally: Source -> AGC -> FLL -> Clock Sync (1 SPS) -> Costas Loop
