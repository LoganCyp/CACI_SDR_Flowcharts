...
class dual_pole_rx(gr.top_block, Qt.QWidget):
    def __init__(self):
        ...
        # --- NEW: QPSK Linear Equalizer from GRC ---
        # Defines the CMA algorithm: step_size=0.01, modulus=1.0
        self.adpt_alg_qpsk = digital.adaptive_algorithm_cma(0.01, 1.0)
        # Defines the Equalizer: 15 taps, 2 SPS, CMA alg, adapt_after_training=True
        self.linear_eq_qpsk = digital.linear_equalizer(
            15, 2, self.adpt_alg_qpsk, True, [], "corr_est"
        )
        ...
        # Connections
        # Channel 0 (QPSK) now flows: Source -> AGC -> FLL -> Clock Sync -> Linear EQ -> Costas Loop
        
        self.connect((self.uhd_usrp_source_0, 0), self.agc_0, self.fll_0, self.sync_0, self.linear_eq_qpsk, self.costas_0)
        # Channel 1 (BPSK) flows normally
